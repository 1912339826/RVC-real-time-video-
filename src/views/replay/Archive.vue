    ...
    <div class="playback_snap_you">
      <div class="snap_you_top">
        ...
        <div class="snap_you_topbutt">
          <el-button class="snap_you_topbutt1" @click="getCheckedNodes">{{
            $t("message.archive.search")
          }}</el-button>
          <el-button class="snap_you_topbutt2" @click="tableDatak">{{
            $t("message.archive.Clear")
          }}</el-button>
        </div>
      </div>
      <div class="snap_you_bottom">
        ...
        <el-table
          ...
          style="width: 100%"
        >
          <el-table-column prop="name" :label="label.Name" min-width="50">
          </el-table-column>
          <el-table-column prop="token" :label="label.Token"> </el-table-column>
          <el-table-column prop="starf" :label="label.StartTime">
            <template slot-scope="scope">
              <i class="el-icon-time"></i>
              <span>{{ scope.row.starf }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="end" :label="label.EndTime">
            <template slot-scope="scope">
              <i class="el-icon-time"></i>
              <span>{{ scope.row.end }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="end" :label="label.Type" min-width="50">
            <template slot-scope="scope">
              <span>{{ scope.row.type }}</span>
            </template>
          </el-table-column>
          <el-table-column fixed="right" width="310px">
            <template slot-scope="scope">
              <div class="snap_you_lable_butt">
                ...
                <el-tooltip
                  ...
                  placement="top-start"
                >
                  <a :href="scope.row.url" :download="scope.row.urlto"
                    ><el-button
                      size="mini"
                ...
                >
                  <el-button
                    size="mini"
                    @click="Refresh1(scope.$index, scope.row)"
                    data-toggle="modal"
                    data-target="#myModal"
    ...
    >
      ...
      <div class="text-center">
        ...
        <div
          ...
          @mouseup="timelinnup($event)"
        >
          <el-slider
            v-model="timelink"
            :max="max"
            @change="timelinn(timelink)"
            :show-tooltip="false"
          ></el-slider>
  ...
  methods: {
    ...
    GetNodeList() {
      ...
      this.$http
        ...
        .catch((error) => {
          console.log("cameraGroup/cameraGroup/getTree");
        });
    },
    to_data(array) {
    ...
    GetsrcList(strNodeId) {
      let root = this.$store.state.IPPORT;
      var url = root + "/api/cluster/v2/GetsrcList?nodeid=" + strNodeId + "";
      this.$http
        .get(url)
        .then((result) => {
    ...
    Refresh(row) {
      ...
      this.rerow = row;
      this.Refreshset = setInterval(
        function () {
          var url1 =
            this.$store.state.IPPORT +
            "/api/v1/GetArchiveStatus?token=" +
            row.token +
            "&filename=" +
            row.strFileName +
            "&session=" +
    ...
    async getCheckedNodes() {
      ...
      var srartdate = this.totime(this.startvalue);
      var enddate = this.totime(this.endvalue);
      for (var l = 0; l < nodes.length; l++) {
        if (!!nodes[l].token && !!nodes[l].id & !!nodes[l].code) {
          let id = [nodes[l].id]
          ...
          // var url =
          //   this.$store.state.IPPORT +
          //   "/api/v1/SearchDeviceRecordByTime?token=" +
          //   nodes[l].token +
          //   "&start=" +
          //   srartdate +
          //   "&end=" +
          ...
          //   });
          var url =
            `http://192.168.5.168:8080/speed-china/camera/camera/getReplayList` +
            `?cameraIds=${id}&date=1` +
            `&startTime=${srartdate}&endTime=${enddate}`;
          console.log(url);
          await this.$http
            .get(url)
            .then((result) => {
    ...
    totime(en) {
      let getMonthCHU = new Date(en).getMonth() + 1;
      let getDateCHU = new Date(en).getDate();
      let getFullYear = new Date(en).getFullYear();
      let getMonth = getMonthCHU < 10 ? "0" + getMonthCHU : getMonthCHU;
      let getDate = getDateCHU < 10 ? "0" + getDateCHU : getDateCHU;
      let getHours = new Date(en).getHours();
      let getMinutes = new Date(en).getMinutes();
      let getSeconds = new Date(en).getSeconds();
