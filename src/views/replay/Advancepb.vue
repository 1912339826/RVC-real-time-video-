        ...
        <div id="videoPanel">
          ...
          >
            ...
            >
              <avintercoms-play
                :h5id="'h' + r + c"
                :h5videoid="'gaovideohb' + r + c"
              ></avintercoms-play>
              <div class="h5controls" style="display:none padding:0px;">
                <button
                  type="button"
                  class="vidbuttion pull-right iconfont icon-roundclosefill"
                  @click="CloseVideo($event)"
                ></button>
                <button
                  type="button"
                  class="vidbuttion pull-right iconfont icon-full"
                  @click="panelFullScreen($event)"
                ></button>
              </div>
            </div>
          </div>
            ...
            <div class="tab" :id="'tab' + r + c">
              ...
              >
                <div class="mode_Choice">
                  ...
                  >
                    <div id="back_Choice" class="back_Choice1">
                      <div
                        :class="{ co_Baise: Adswitch == 'true' }"
                        @click="Adswitchs"
                        class="co_black"
                      >
                        {{ $t("message.playback.H5SPlayback") }}
                      </div>
                      <div
                        :class="{ co_Baise: Adswitch == 'false' }"
                        @click="Adswitchs1"
                        class="co_black"
                      >
                        {{ $t("message.playback.NVRPlayback") }}
                      </div>
                    </div>
                ...
                >
                  <button
                    type="button"
                    class="layout layout1x1"
                    data-row="1|1"
                    @click="changePanel($event)"
                  ></button>
                  <button
                    type="button"
                    class="layout layout2x2"
                    data-row="2|2"
                    @click="changePanel($event)"
                  ></button>
                  <i
                    style="font-size: 24px; color: #b2b1b1"
                    class="layoutfull"
                    @click="panelFullScreen($event)"
                  ></i>
                  <el-date-picker
                ...
                <div class="caveat_butt" style="">
                  <button class="mr-1" type="button"></button
                  >{{ $t("message.archive.ManualRecord") }}
                  <button class="mr-2" type="button"></button
                  >{{ $t("message.archive.AlarmRecord") }}
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    timetz() {
      ...
      setTimeout(function () {
        var a = $("#timeline" + _this.selectRow + _this.selectCol).TimeSlider(
          "returnMouseupTime",
          null,
          null,
          function (time) {
            ...
            if (_this.Adswitch == "false") {
              url = url =
                ...
                _this.Gtoken +
                "&start=" +
                encodeURIComponent(timevalues1) +
                "&end=" +
                encodeURIComponent(timevaluee1) +
                "&session=" +
                _this.$store.state.token;
            } else {
              url =
                root +
                "api/v1/Search?type=record&token=" +
                _this.Gtoken +
                "&start=" +
                encodeURIComponent(timevalues1) +
                "&end=" +
                encodeURIComponent(timevaluee) +
                "&session=" +
                _this.$store.state.token;
            }
            // console.log(url);
            ...
            _this.$http.get(url).then((result) => {
              if (result.status == 200) {
                ...
                for (var i = 0; i < data.record.length; i++) {
                  ...
                  $("#timeline" + _this.selectRow + _this.selectCol).TimeSlider(
                    "init",
                    timevalue,
                    timedata1
                  );
        ...
        function () {
          if (this.selectRow == "1" && this.selectCol == "1") {
            if (this.v1 != undefined) {
              ...
              $("#timeline" + this.selectRow + this.selectCol).TimeSlider(
                "init",
                timevalue,
                timedata1
              );

              $("#gaovideohb" + this.selectRow + this.selectCol)
                .get(0)
                .load();
    ...
    updateUI() {
      ...
      if ($(document.body).width() < 768) {
        ...
        this.contentHeight = window.screen.height * 0.65;
      }
      ...
      .videoColor {
        ...
        .palace {
          ...
          .h5controls {
            position: absolute;
            top: 0;
            background: url("~@/views/liveview/imgs/liveview_buttback.png")
              no-repeat;
