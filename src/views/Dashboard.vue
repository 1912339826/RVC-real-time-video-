      ...
      <div class="dashboard_left_bottom">
        ...
        <div class="dashboard_content">
          <el-table
            ...
            style="width: 100%"
          >
            <el-table-column prop="strNodeId" label="编号"> </el-table-column>
            <el-table-column prop="strNodeName" label="名称"> </el-table-column>
              ...
              <template slot-scope="scope">
                <div style="width: 100%; text-align: right">
                  <el-button
                    type="text"
                    size="small"
                    @click="Nodedetails(scope.$index, scope.row)"
                    >详情</el-button
                  >
                </div>
              </template>
    ...
    <div class="dashboard_center">
      ...
      <div class="dashboard_center_bottom">
        ...
        <div class="dashboard_left_bottom_Devresources">
          <div
            class="containesr_sdks"
            v-for="(da, index) in devbucket"
            :key="index"
          >
            <div class="sdk_back">
          ...
          >
            ...
            <el-table-column label="状态" align="right">
              <template slot-scope="scope">
                <!-- {{scope.row.bOnline}} -->
                <i
                  :class="scope.row.bOnline ? 'color_green' : 'color_red'"
                  class="iconfont icon-hanbaobao"
                ></i>
              </template>
            </el-table-column>
            <!-- <el-table-column>
...
export default {
  ...
  methods: {
    ...
    Nodedetails(index, row) {
      ...
      var url =
        root + "/api/cluster/v2/GetNodeInfo?nodeid=" + row.strNodeName + "";
      this.$nextTick(() => {
        // 波动图
        _this.liunv();
    ...
    liunv() {
      ...
      for (var i = 1; i < 62; i++) {
        ...
        date.push([("0" + now.getSeconds()).slice(-2) + "s"]);
      }
      // return false;
      // 绘制图表
      var titlecol;
      ...
      var Option = {
        tooltip: {
          ...
          borderWidth: 0,
          formatter:
            "{b0}<br />{a0}:{c0}Kbps<br />{a1}:{c1}Kbps <br />{a2}:{c2}%",
        },
        title: {
          left: "center",
            ...
            var nodedatafor = [
              {
                ...
                percentage: Number(
                  (
                    (byte.nTotalSpaceByte - byte.nFreeSpaceByte) /
                    byte.nTotalSpaceByte
                  ).toFixed(2)
                ),
                value:
                  (byte.nFreeSpaceByte / 1024 / 1024 / 1024).toFixed(1) + "G",
              },
              {
                id: clusterid1,
                ...
                value:
                  (byte.nTotalMemoryByte / 1024 / 1024 / 1024).toFixed(1) + "G",
              },
              {
                id: clusterid2,
                ...
                percentage: Number(
                  (
                    (byte.nRecordTotalSpaceByte - byte.nRecordFreeSpaceByte) /
                    byte.nRecordTotalSpaceByte
                  ).toFixed(2)
                ),
                value:
                  (byte.nRecordTotalSpaceByte / 1024 / 1024 / 1024).toFixed(1) +
                  "G",
              },
            ];
        ...
        .then((result) => {
          ...
          if (result.status == 200) {
            ...
            if (result.data.nCloudTotal == 0) {
              ...
              Cloud = Math.round(
                (result.data.nCloudOnline / result.data.nCloudTotal) * 100
              );
            }
            if (result.data.nRTSPRTMPTotal == 0) {
              ...
              RTSPRx = Math.round(
                (result.data.nRTSPRTMPOnline / result.data.nRTSPRTMPTotal) * 100
              );
            }
            ...
            if (result.data.nONVIFTotal == 0) {
              ...
              ONVIFx = Math.round(
                (result.data.nONVIFOnline / result.data.nONVIFTotal) * 100
              );
            }
            ...
            if (result.data.nRTMPPushTotal == 0) {
              ...
              RTMPx = Math.round(
                (result.data.nRTMPPushOnline / result.data.nRTMPPushTotal) * 100
              );
            }
            if (result.data.nSdkTotal == 0) {
              ...
              DevSDK = Math.round(
                (result.data.nSdkOnline / result.data.nSdkTotal) * 100
              );
            }
            this.devbucket = [
              {
            ...
            //显示圆环
            var number = (
              result.data.nCameraOnline / result.data.nCameraTotal
            ).toFixed(2);
            // for(var i = 0; i<data.length; i++){
            this.Monitoringnumber(
              clusterid,
              "1",
              number,
              result.data.nCameraTotal
            );
            // }
          }
        })
...
.Cluster_dashboard {
  ...
  .dashboard_left {
    ...
    .dashboard_left_top {
      ...
      background: url("~@/views/imgs/dashboard_back_img.png") no-repeat center
        center;
    ...
    .dashboard_left_bottom {
      ...
      height: 63%;
      // border: 1px solid #ffffff;
      background: url("~@/views/imgs/dashboard_back_img3.png") no-repeat center
        center;
      background-size: 100% 100%;
      .dashboard_content {
        width: 100%;
  ...
  .dashboard_center {
    ...
    .dashboard_center_top {
      ...
      height: 66%;
      // border: 1px solid #ffffff;
      background: url("~@/views/imgs/dashboard_back_img2.png") no-repeat center
        center;
      ...
      .center_top_event {
        ...
        height: 106px;
        font-size: 13px;
        background: url("~@/views/imgs/dashboard_back_imgevent.png") no-repeat
          center center;
        background-size: 100% 100%;
        position: absolute;
        left: 25%;
      ...
      .center_top_node {
        ...
        height: 156px;
        font-size: 16px;
        background: url("~@/views/imgs/dashboard_back_imgnode.png") no-repeat
          center center;
        background-size: 100% 100%;
        position: absolute;
        left: 10%;
    ...
    .dashboard_center_bottom {
      ...
      height: 32%;
      // border: 1px solid #ffffff;
      background: url("~@/views/imgs/dashboard_back_img4.png") no-repeat center
        center;
      background-size: 100% 100%;
      .dashboard_left_bottom_Devresources {
        ...
        .containesr_sdks {
          ...
          .sdk_back {
            width: 100%;
            height: 78%;
            background: url("~@/views/imgs/dashboard_back_bucket.png") no-repeat
              center center;
            background-size: 100% 100%;
            margin-bottom: 10px;
            line-height: 130px;
  ...
  .dashboard_right {
    ...
    .dashboard_right_top {
      ...
      height: 38%;
      // border: 1px solid #ffffff;
      background: url("~@/views/imgs/dashboard_back_img1.png") no-repeat center
        center;
      background-size: 100% 100%;
      .color_red {
        color: #ff564f;
